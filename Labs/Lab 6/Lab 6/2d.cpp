#include <gl\glut.h>
#include <math.h>
#include <E:\study\2 course\4 Semester\Computer Graphic\Комп.графика\glaux.h>
#include <stdio.h>
#pragma comment(lib, "E:\\study\\2 course\\4 Semester\\Computer Graphic\\Комп.графика\\GLAUX.lib")
...
GLint SohranenieIzobrajeniyaVfail(const char* szFileName)
{
	...
	Izobrajenie* iz = (Izobrajenie*)malloc(sizeof(Izobrajenie)); GLint
		iViewport[4]; //Массив для хранения размеров порта просмотра
...
void Pererisovka(void)
{
	...
	case 1: /*Загрузка изображения из файла *.bmp*/
		pImage = auxDIBImageLoadA("E:/study/2 course/4 Semester/Computer Graphic/Комп.графика/texture_4.bmp");
		...
		izobr_bmp->zagolovok.kol_voKomponent = 3; izobr = izobr_bmp;
		break;
	case 2:
		...
		break;
	case 3:
		glPixelZoom(1, -1); //задает коэффициенты растяжения и сжатия для операций записи пикселей
		glRasterPos2d(0, 512); //задает текущую позицию растра
		break;
	case 4:
	...
	case 5: /*Инверсия цветов*/
		...
		glPixelMapfv(GL_PIXEL_MAP_R_TO_R, 255, invertMap); //загружает пиксельную карту (модифицикация с помощью таблиц подстановки)
		glPixelMapfv(GL_PIXEL_MAP_G_TO_G, 0, invertMap);
		glPixelMapfv(GL_PIXEL_MAP_B_TO_B, 0, invertMap);
...
int main(int argc, char* argv[])
{
	...
	glutCreateMenu(ObrabotkaMenu); glutAddMenuEntry("clear screen", 0);
	glutAddMenuEntry("Download.bmp", 1);
	glutAddMenuEntry("Save current image", 2);
	glutAddMenuEntry("Flip the image about the x-axis", 3);
	glutAddMenuEntry("B display only", 4);
	glutAddMenuEntry("G inversion", 5);
