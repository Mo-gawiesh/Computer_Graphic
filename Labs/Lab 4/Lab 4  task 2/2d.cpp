...
void fig()//рисование полигональной сетки
{
	...
	glNormal3f(0, 0, -1);//устанавливает «текущую нормаль», которая применяется ко всем вершинам,последовательно
		...
		glVertex3f(0, 0, 0); glVertex3f(0, 1, 0); glVertex3f(2, 0, 0);
	glEnd();
	glBegin(GL_POLYGON); //2 передняя
	glNormal3f(0, 0, 2);
	glVertex3f(0, 0, 1); glVertex3f(0, 1, 1); glVertex3f(2, 0, 1);
	glEnd();
	glBegin(GL_POLYGON);//3 левая
	glNormal3f(-1, 0, 0);
	glVertex3f(0, 0, 1); glVertex3f(0, 1, 1); glVertex3f(0, 1, 0); glVertex3f(0, 0, 0);
	glEnd();
	glBegin(GL_POLYGON);//4 нижняя
	glNormal3f(0, -1, 0);
	glVertex3f(0, 0, 1); glVertex3f(0, 0, 0); glVertex3f(2, 0, 0); glVertex3f(2, 0, 1);
	glEnd();
	glBegin(GL_POLYGON);//5 правая
	glNormal3f(0.5, 2, 0);
	glVertex3f(0, 1, 1); glVertex3f(0, 1, 0); glVertex3f(2, 0, 0); glVertex3f(2, 0, 1);
	glEnd();
}
void myDisplay()
{
	...
	*/
	...
	GLfloat myDiffuse[] = { 0.4,0.4,0.4, 1 }; //часть падающего света слегка проникает внутрьповерхности
		...
		GLfloat myAmbient[] = { 0.25, 0.25, 0.25, 1 }; //фоновый свет
	GLfloat mySpecular[] = { 0.774597, 0.774597, 0.774597, 1 }; //больше похожи на зеркало и имеют ярко
	...
	glLightfv(GL_LIGHT0, GL_POSITION, myLightPosition);// Здесь изменим конкретный источник света вCKe
		glBegin(GL_LINES);
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("Lab 4 task 2 Gawish"); //инициализируется открытие экранного окна window
